function [movingweights, output]= threshold_linear_network(inputs, initialweights, epsilon, threshold)
...
%change in weight is equal to epsilon*postsynaptic excitation*(new
%input-weight), when excitation is above threshold output is linear
        ...
        for i= 1: num_inputs
            ...
            if output(1,i) > threshold
                for j = 1: input_dimensions
                    movingweights(j, lengthw+1) = movingweights(j, lengthw) + (epsilon*output(1,i)*(inputs(j,i)-movingweights(j,lengthw)));
                end
            end
        end
    end
